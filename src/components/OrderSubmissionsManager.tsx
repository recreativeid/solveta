...
export const OrderSubmissionsManager: React.FC<{
  ...
  onConvertToProject?: (submission: CustomerOrderSubmission) => void;
}> = ({ showToast, onConvertToProject }) => {
  ...
  const getFormUrl = () => {
    if (typeof window === "undefined") return "/formulir/";
    const path = window.location.pathname;
    const base = path.startsWith("/solveta") ? "/solveta" : "";
  ...
  const copyFormLink = () => {
    if (typeof window !== "undefined") {
      ...
      showToast("Link Formulir Customer berhasil disalin ke clipboard!");
    }
  };
    ...
    <div className="space-y-6 bg-white font-sans text-left">
      ...
      <div className="bg-white border border-gray-200/80 rounded-xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <a
            href={typeof window !== "undefined" ? getFormUrl() : "/formulir/"}
            onClick={(e) => {
              e.preventDefault();
              window.open(getFormUrl(), "_blank");
            }}
            className="px-3.5 py-1.5 bg-white hover:bg-gray-50 border border-gray-200 text-gray-900 text-xs font-semibold rounded-lg transition-colors flex items-center gap-1.5 cursor-pointer shadow-xs"
            title="Buka Formulir Customer di tab baru"
          >
                    ...
                    <td className="py-3 px-4 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end gap-1.5">
                        {onConvertToProject && (
                          <button
                            type="button"
                            onClick={() => onConvertToProject(sub)}
                            className="px-2.5 py-1 bg-gray-900 hover:bg-black text-white text-xs font-medium rounded-lg transition-colors flex items-center gap-1 cursor-pointer"
                            title="Tarik isian formulir ini ke menu Pencatatan Proyek & Invoice"
                          >
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                  {onConvertToProject && (
                    <button
                      ...
                      className="w-full sm:w-auto px-4 py-2 bg-gray-900 hover:bg-black text-white text-xs font-medium rounded-lg transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                    >
                      <FileText className="w-3.5 h-3.5" />
                      <span>Jadikan Catatan Proyek</span>
                    </button>
                  )}
                  ...
                  <button
                    ...
                    className="w-full sm:w-auto px-4 py-2 bg-white hover:bg-gray-50 border border-gray-200 text-gray-700 text-xs font-medium rounded-lg transition-colors cursor-pointer"
                  >
                    Tutup
                  </button>
                </div>
