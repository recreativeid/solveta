...
export const ClientMarquee: React.FC = () => {
  ...
  const logoHeight = data.siteCopy.marqueeLogoHeight || 65;
  const logoSpacing = data.siteCopy.marqueeLogoSpacing || 40;
  const logoScalePercent = (data.siteCopy.marqueeLogoScale || 100) / 100;
  const logoMaxWidth = data.siteCopy.marqueeLogoMaxWidth || 350;
  ...
  const renderBrandPill = (brand: ClientBrandItem, index: number) => {
    ...
    const finalHeight = Math.max(28, Math.round(logoHeight * effectiveScale));
    const finalMaxWidth = Math.max(100, Math.round(logoMaxWidth * effectiveScale));
        ...
        >
          <img
            ...
            style={{
              ...
              maxHeight: `${Math.round(finalHeight * 1.45)}px`,
              maxWidth: `${finalMaxWidth}px`,
            }}
            className="w-auto object-contain select-none pointer-events-none filter grayscale opacity-85 contrast-125 dark:invert dark:opacity-95 dark:contrast-125 group-hover:grayscale-0 group-hover:dark:invert-0 group-hover:opacity-100 group-hover:scale-105 transition-all duration-300"
          />
        </div>
      );
    }
  ...
  return (
    <section className="py-12 sm:py-16 bg-white dark:bg-[#07080E] border-y border-gray-100 dark:border-gray-800 overflow-hidden relative transition-colors duration-200">
